...
static int nrfs_irq_register_isr(__u32 pin, __u32 confg, IRQ_DEV_HANDLER isr, void *dev)
{
	uint32_t err_code;
	nrf_drv_gpiote_in_config_t config = {0};
	struct irq_driver *idrv = (struct irq_driver *)dev;

	config.is_watcher = IS_WATCHER(confg) ? true : false;
	config.hi_accuracy = IS_HI_ACCURACY(confg) ? true : false;

	if (IS_NOPULL(confg))
		config.pull = NRF_GPIO_PIN_NOPULL;
	else if (IS_PULLDOWN(confg))
		config.pull = NRF_GPIO_PIN_PULLDOWN;
	else if (IS_PULLUP(confg))
		config.pull = NRF_GPIO_PIN_PULLUP;

	if (IS_LEVEL_HIGH(confg))
		config.sense = NRF_GPIOTE_INITIAL_VALUE_HIGH;
	else if (IS_LEVEL_LOW(confg))
		config.sense = NRF_GPIOTE_INITIAL_VALUE_LOW;
	else if (IS_TRIGGLE(confg)) {
		if (IS_TRIGGLE_RISING(confg) && IS_TRIGGLE_FALLING(confg))
			config.sense = NRF_GPIOTE_POLARITY_TOGGLE;
		else if (IS_TRIGGLE_RISING(confg))
			config.sense = NRF_GPIOTE_POLARITY_LOTOHI;
		else if (IS_TRIGGLE_FALLING(confg))
			config.sense = NRF_GPIOTE_POLARITY_HITOLO;
	}

	err_code = nrf_drv_gpiote_in_init(pin, &config, isr);
	idrv->pin = pin;
	idrv->confg = confg;
...
static void nrfs_irq_mask(__u32 pin)
{
	nrf_drv_gpiote_in_event_enable(pin, false);
	nrf_drv_gpiote_in_event_disable(pin);
}

static void nrfs_irq_unmask(__u32 pin)
{
	nrf_drv_gpiote_in_event_enable(pin, true);
}

static struct irq_driver nrfs_irq_driver = {
	.type = "nrfs_irq",
	.irq_register_isr = nrfs_irq_register_isr,
	.int_ctrl = {
		.mask = nrfs_irq_mask,
		.unmask = nrfs_irq_unmask,
	}
};

int __init nrfs_irq_init(void)
{
	ret_code_t err_code;
	...
	return irq_register_driver(&nrfs_irq_driver);
}
#endif
