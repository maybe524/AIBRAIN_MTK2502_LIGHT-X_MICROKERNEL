...
#ifdef CONFIG_KERNEL_SYSTEM
ret_code_t nrf_drv_spi_attach_master(nrf_drv_spi_t const * const p_instance,
							struct spi_master *master)
{
...
#ifdef CONFIG_KERNEL_SYSTEM
	if (p_cb->master) {
		struct spi_slave_info *info = p_cb->master->active_slave->slave_info;

		if (info->mode == SPI_MODE_4WIRE_1DAT_1DC) {
			if (GPIO_PIN(info->chip_select) != SPI_UNSUE_PIN)
				gpio_set_out(info->chip_select, GPIO_OUT_ONE);
			if (GPIO_PIN(info->dc_select) != SPI_UNSUE_PIN && info->status == SPI_WRITE_CMD)
				gpio_set_out(info->dc_select, GPIO_OUT_ONE);
		}

		unlock(&p_cb->master->lock);
	}
#endif
                     ...
                     nrf_drv_spi_xfer_desc_t const * p_xfer_desc)
{
	...
	if (p_cb->master) {
		...
		if (info->mode == SPI_MODE_4WIRE_1DAT_1DC) {
			if (GPIO_PIN(info->chip_select) != SPI_UNSUE_PIN)
				gpio_set_out(info->chip_select, GPIO_OUT_ZERO);
			if (GPIO_PIN(info->dc_select) != SPI_UNSUE_PIN && info->status == SPI_WRITE_CMD)
				gpio_set_out(info->dc_select, GPIO_OUT_ZERO);
		}
	}
#endif
