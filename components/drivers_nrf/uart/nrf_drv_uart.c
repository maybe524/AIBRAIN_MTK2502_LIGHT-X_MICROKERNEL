...
extern char uart_rxdat[100];
extern unsigned char uart_rxlen;
extern bool uart_data_ready, uart_buff_release;
extern void lock_uart_buf(void);
extern void unlock_uart_buf(void);
extern kd_bool m_uart_data_ready;
extern kd_bool system_inited;

DECLARE_WAIT_QUEUE_HEAD(WAIT_QUEUE_UART, uart_waiter_queue);
#endif
...
__STATIC_INLINE void uart_irq_handler()
{
...
#ifdef CONFIG_KERNEL_SYSTEM
		if (system_inited) {
			ksprintf(msg_temp, "MT%cULS:ON\r\n", CMD_MT2502_LSW);
			msg_create(CMD_UART, CMD_MT2502_MSG, MSG_TO_RECEIVER_ONLINE | MSG_NO_REPEAT, msg_temp, 11);
		}
#endif
    }
             ...
             nrf_uart_event_check(NRF_UART0, NRF_UART_EVENT_RXDRDY))
    {
		...
		nrf_uart_event_clear(NRF_UART0, NRF_UART_EVENT_RXDRDY);
		if (uart_data_ready) {
			char chr_tmp = nrf_uart_rxd_get(NRF_UART0);
			return;
		}
		
		if (uart_buff_release) {
			uart_rxlen = 0;
			uart_buff_release = false;
		}
		
		uart_rxdat[uart_rxlen++] = nrf_uart_rxd_get(NRF_UART0);
		if (uart_rxdat[0] == '.') {
			if (uart_rxlen >= 2) {
				if (uart_rxdat[1] == '/') {
					if (uart_rxdat[uart_rxlen - 1] == '\n' && uart_rxdat[uart_rxlen - 2] == '\r' || \
							uart_rxlen >= sizeof(uart_rxdat)) {
						lock_uart_buf();
						msg_create(CMD_UART, CMD_SHELL, MSG_TO_RECEIVER_ONLINE, uart_rxdat, uart_rxlen);
						wake_up_interruptible(&uart_waiter_queue);
						unlock_uart_buf();
					}
				}
				else
			...
			if (uart_rxlen >= 2) {
				if (uart_rxdat[1] == 'T') {
					if (uart_rxdat[uart_rxlen - 1] == '\n' && uart_rxdat[uart_rxlen - 2] == '\r' || \
						...
						msg_create(CMD_UART, CMD_MT2502_MSG, MSG_TO_RECEIVER_ONLINE, uart_rxdat, uart_rxlen);
						wake_up_interruptible(&uart_waiter_queue);
						unlock_uart_buf();
					}
				}
...
#ifdef CONFIG_KERNEL_SYSTEM
int __init nrfs_drv_uart_init(void)
{
	return wait_queue_register(&uart_waiter_queue);
}
#endif