...
#define BOOT_VER_NUM			0x16062801 
#endif
...
static void bootloader_settings_save(bootloader_settings_t * p_settings)
{
    ...
    APP_ERROR_CHECK(err_code);
#ifdef BOOT_VER_NUM_SUPPORT
	p_settings->boot_version = BOOT_VER_NUM;
#endif
...
#ifdef BOOT_VER_NUM_SUPPORT
	p_settings->boot_version   = p_bootloader_settings->boot_version;
#endif
}

#ifdef BOOT_VER_NUM_SUPPORT
bootloader_settings_t m_bootloader_settings;
bool check_boot_ver_num(void)
{	
	bootloader_settings_get(&m_bootloader_settings);
	
	if (BOOT_VER_NUM != m_bootloader_settings.boot_version) {
		bootloader_settings_save(&m_bootloader_settings);
		return false;
	} else {
		return true;
	}
