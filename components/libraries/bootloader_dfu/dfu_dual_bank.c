...
#ifdef BOOT_VER_NUM_SUPPORT
APP_TIMER_DEF(m_boot_num_timer_id);                             /**< Application timer id. */
#define BOOT_NUM_TIMEOUT_INTERVAL	APP_TIMER_TICKS(2000, APP_TIMER_PRESCALER)
#endif
...
#ifdef BOOT_VER_NUM_SUPPORT
bool m_boot_ver_num_flag = false;

static void boot_num_timeout_handler(void * p_context)
{
	m_boot_ver_num_flag = false;
}

uint32_t boot_num_timer_start(void)
{
    uint32_t err_code;
	
	err_code = app_timer_create(&m_boot_num_timer_id,
							APP_TIMER_MODE_SINGLE_SHOT,
							boot_num_timeout_handler);
    APP_ERROR_CHECK(err_code);
	
	err_code = app_timer_start(m_boot_num_timer_id, BOOT_NUM_TIMEOUT_INTERVAL, NULL);
    APP_ERROR_CHECK(err_code);
