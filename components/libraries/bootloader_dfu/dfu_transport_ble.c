...
#define DEVICE_NAME                          "PagerOTA"                                               /**< Name of device. Will be included in the advertising data. */
#define MANUFACTURER_NAME                    "37body"                                   /**< Manufacturer. Will be passed to Device Information Service. */
...
static void gap_params_init(void)
{
    ...
    if ((NRF_UICR->CUSTOMER[0] != 0xFFFFFFFF) && (NRF_UICR->CUSTOMER[1] != 0xFFFFFFFF)) {
        gap_addr.addr[0] = NRF_UICR->CUSTOMER[0] & 0xFF;
        gap_addr.addr[1] = (NRF_UICR->CUSTOMER[0] >> 8) & 0xFF;
        gap_addr.addr[2] = (NRF_UICR->CUSTOMER[0] >> 16) & 0xFF;
        gap_addr.addr[3] = (NRF_UICR->CUSTOMER[0] >>24) & 0xFF;
        gap_addr.addr[4] = NRF_UICR->CUSTOMER[1] & 0xFF;
        gap_addr.addr[5] = (NRF_UICR->CUSTOMER[1] >> 8) & 0xFF;
        gap_addr.addr_type = (NRF_UICR->CUSTOMER[1] >> 24) & 0xFF;
