...
static app_button_cfg_t nrfs_button_info = {0};

uint32_t nrfs_button_register(struct button_info *button_info, __u8 button_count, __u32 detection_delay)
{
	nrfs_button_info.pin_no = button_info->pin_no;
	nrfs_button_info.active_state = button_info->active_state;
	nrfs_button_info.pull_cfg = button_info->pull_cfg;
	nrfs_button_info.button_handler = button_info->button_handler;

	return app_button_init(&nrfs_button_info, 1, detection_delay);
}

static struct button_driver nrfs_button_driver = 
{
	.button_register = nrfs_button_register,
	.button_enable = app_button_enable,
};

int __init nrfs_button_init(void)
{
	return button_driver_register(&nrfs_button_driver);
}
