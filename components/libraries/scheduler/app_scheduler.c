...
static __sched int
nrfs_sched(struct task_struct **current)
{
	app_sched_execute();
...
static int
nrfs_sched_put(struct task_struct *task, __u8 config)
{
	app_sched_event_put(task->data,
				task->t_param,
				(app_sched_event_handler_t)task->task_handler);
	return 0;
}

static int
nrfs_sched_set(struct task_struct *task, __u8 config)
{
	return 0;
}

static struct task_sched task_sched_nrf = {
	.type = SCHED_NRFS_TYPE,
	.sched = nrfs_sched,
	.sched_put = nrfs_sched_put,
	.sched_set = nrfs_sched_set,
};

int __init nrfs_sched_init(void)
{
	APP_SCHED_INIT(20, SCHED_QUEUE_SIZE);

	return sched_register(&task_sched_nrf);
}
#endif
