...
static ssize_t fat_read_block(struct fat_fs *fs, 
		void *buff, int blk_no, __size_t off, __size_t size)
{
	struct block_device *bdev = fs->bdev;
	struct disk_drive *drive = container_of(bdev, struct disk_drive, bdev);

	__size_t buf_len = (off + size + drive->sect_size - 1) & ~(drive->sect_size - 1);
...
static __u32 fat_get_fat_table(struct fat_fs *fs, __u32 fat_num)
{
	...
	if (old_fat == ~0 || fat_num < old_fat || \
			fat_num > (fs->clus_size / sizeof(fat_num) + old_fat - 1)) {
		ret = fat_read_block(fs, fat_catch,
			fs->dbr.resv_size / fs->dbr.sec_per_clus + fat_num * sizeof(fat_num) / (fs->clus_size),
			0, fs->clus_size);
...
static struct dentry *fat_mount(struct file_system_type *fs_type, 
		unsigned long flags, struct block_device *bdev)
{
	int ret;
	__u16 blk_size;
	__u32 clus_size;
	...
	fat_fs = (struct fat_fs *)u_zalloc(sizeof(*fat_fs));
	if (fat_fs == NULL) {
		DPRINT("%s(): no mem\r\n", __func__);
		return NULL;
	}
	...
	// fixme
	root_ino = (struct inode *)u_zalloc(sizeof(*root_ino));
	if (!root_ino)
		return NULL;
	...
	root_dir = (struct dentry *)u_zalloc(sizeof(*root_dir));
	if (!root_dir)
		return NULL;

	root_dir->d_inode = root_ino;
...
static struct dentry *fat_lookup(struct inode *parent, const char *name)
{
	...
	fat_de = (struct fat_dentry *)u_malloc(sizeof(*fat_de));
	if (fat_de == NULL) {
		DPRINT("%s(): u_malloc err\r\n", __func__);
		return NULL;
	}
	...
	while (1) {
		...
		if (u_strcmp(sname, name) == 0) {
			...
			ino = (struct inode *)u_zalloc(sizeof(*ino));
			if (!ino)
				return NULL;
			...
			de = (struct dentry *)u_zalloc(sizeof(*de));
			if (!de)
				return NULL;
...
static ssize_t fat_read(struct file *fp, void *buff, __size_t size, loff_t *off)
{
	__u32 clus_num, clus_size;
	__size_t pos;
	__size_t count = 0;
	__size_t tmp_size;
	struct fat_fs *fs = fp->f_dentry->d_inode->i_fs;
	struct fat_dentry *de = (struct fat_dentry *)fp->f_dentry;
...
static ssize_t fat_write(struct file *fp, const void *buff, __size_t size, loff_t *off)
{
	return 0;
}
