...
static int kvsprintf(char *buf, const char *fmt, int *para)
{
...
int kprintf(const char *fmt, ...)
{
	...
	prted_len = kvsprintf(array_buf, fmt, arg);
...
static int kvsnprintf(char *buf, int size, const char *fmt, int *para)
{
...
int ksnprintf(char *buf, __size_t size, const char *fmt, ...)
{
	int *arg = (int *)&fmt + 1;
	int printed;

	printed = kvsnprintf(buf, size, fmt, arg);
