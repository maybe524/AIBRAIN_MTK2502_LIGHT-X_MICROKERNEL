...
extern	bool	m_boot_ver_num_flag;
extern	bool	check_boot_ver_num(void);
extern	uint32_t boot_num_timer_start(void);
#endif
...
int main(void)
{
	...
	if (NRF_POWER->GPREGRET == 0x80) {
		dfu_start |= true;
	}		
	
#ifdef BOOT_VER_NUM_SUPPORT
	if (!check_boot_ver_num()) {
		m_boot_ver_num_flag = true;
		boot_num_timer_start();
		for (;;) {
			if (!m_boot_ver_num_flag)
				break;
